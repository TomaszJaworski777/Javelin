...
impl Node {
    ...
    pub fn print_node(
        ...
        let p_text = format!(
            "P({})",
            heat_color(
                format!("{:.2}%", self.policy_value * 100.0).as_str(),
                self.policy_value,
                heat_min_value,
                heat_max_value
            )
        ...
        if self.mv == Move::NULL {
            println!("{}{:<28}{:<31}{:<35}", prefix, move_str, q_text, n_text);
        } else if has_promotion {
            println!("{}{:<34}{:<31}{:<35}{}", prefix, move_str, q_text, n_text, p_text);
        } else {
            println!("{}{:<33}{:<31}{:<35}{}", prefix, move_str, q_text, n_text, p_text);
        }
    }
}
