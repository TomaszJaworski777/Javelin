...
pub struct ValueNetwork {
    ...
    hidden_layer: DenseLayer<16, 2, ScReLUActivation>,
    output_layer: DenseLayer<2, 1, ScReLUActivation>,
}
#[allow(unused)]
impl ValueNetwork {
    pub fn set_layer_weights(&mut self, index: usize, weights: Vec<Vec<f32>>) {
        match index {
            ...
            1 => self.hidden_layer.layer_mut().set_weights(weights),
            2 => self.output_layer.layer_mut().set_weights(weights),
            _ => return,
        }
    }
    ...
    pub fn set_layer_biases(&mut self, index: usize, biases: Vec<f32>) {
        match index {
            ...
            1 => self.hidden_layer.layer_mut().set_biases(biases),
            2 => self.output_layer.layer_mut().set_biases(biases),
            _ => return,
        }
    }
    ...
    pub fn print(&self) {
        ...
        self.hidden_layer.layer().print();
        self.output_layer.layer().print();
    }

    pub fn evaluate(&self, board: &Board) -> f32 {
        let input_layer_result = self.input_layer.forward(&board);
        let hidden_layer_output = self.hidden_layer.forward(&input_layer_result);
        let output_layer_result = self.output_layer.forward(&hidden_layer_output);
