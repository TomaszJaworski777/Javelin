        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                if piece_board.num <= 104 {
                    let mut policy_data = ChessPolicyData {
                        board: piece_board,
                ...
                if current_board.is_insufficient_material() || current_board.three_fold() || current_board.half_moves >= 100 {
                    ...
                    if move_list.len() == 0 {
                        game_result = if current_board.is_in_check() { if current_board.side_to_move.current() == 0 { GameResult::Lose(0) } else { GameResult::Win(0) } } else { GameResult::Draw }
                    }
                }
                ...
                if game_result != GameResult::None {
                    ...
                    for item in &mut temp.value_data {
                        item.result = match game_result {
                            GameResult::None => 0,
                            GameResult::Draw => 0,
                            GameResult::Lose(_) => if item.side_to_move == 0 { -1 } else { 1 }, 
                            GameResult::Win(_) => if item.side_to_move == 0 { 1 } else { -1 }
                        }
                    }
                    ...
                    for item in &mut temp.policy_data {
                        item.board.result = match game_result {
                            GameResult::None => 0,
                            GameResult::Draw => 0,
                            GameResult::Lose(_) => if item.board.side_to_move == 0 { -1 } else { 1 }, 
                            GameResult::Win(_) => if item.board.side_to_move == 0 { 1 } else { -1 }
                        }
                    }
